...
frappe.ui.form.on('Job Card', {
	setup: function(frm) {
		frm.set_query("assigned_technician", function() {
			...
			if (frm.doc.device_type) {
				filters.specialization = frm.doc.device_type;
			}
	...
	refresh: function(frm) {
		...
		frm.add_custom_button("Transfer Technician", function() {
			frappe.prompt([
				{ label: "New Technician", fieldname: "technician", fieldtype: "Link", options: "Technician", reqd: 1 }
			], function(values) {
		...
		frappe.call({
			...
			callback: function(r) {
				...
				if (spec != frm.doc.device_type) {
					frappe.msgprint("Warning: Technician specialization does not match device type");
				}
			}
		});
	}
